...
#if defined(NDEBUG)
# error "LOOP cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x06f1857b2a99bd7961af1bad548bf0defce64f59970ebf8e847ead50907df71e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LOOP: starting difficulty is 1 / 2^12
...
{
    // LOOP: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 346500 blocks
    nSubsidy >>= (nHeight / 346500);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LOOP: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LOOP: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1528791043, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "THE TIMES NEW ROMAN";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047ce11ddfa6d2d1a835380c98a2b60391b89bc7c7913067b6e5cf51521db99ae7bdf7dc8201bf4d669dc7c89b1c038f020ddbce0f0b61e136137d4f37412d779e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528791043;
        ...
        assert(block.hashMerkleRoot == uint256("0x8976b5d26615f2deba6143913a635b70ad6b36613f3ccc2bfff427f7781991c6"));
...
unsigned char pchMessageStart[4] = { 0x5d, 0x2d, 0x08, 0x53 }; // LOOP: increase each by adding 2 to bitcoin's value.
...
void static LOOPMiner(CWallet *pwallet)
{
    printf("LOOPMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("loop-miner");
    ...
    try { loop {
        ...
        printf("Running LOOPMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LOOPMiner, pwallet));
}
